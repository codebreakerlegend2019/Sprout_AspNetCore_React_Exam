...
<button type="button" onClick={() => this.props.history.push("/employees/index")} className="btn btn-primary">Back</button>
  ...
  async saveEmployee() {
    ...
    const requestOptions = {
      ...
      await fetch('api/employees/' + this.state.id, requestOptions)
          .then(r => r.json())
          .then(data => {
              console.log(data);
  ...
  async getEmployee(id) {
    ...
    const response = await fetch('api/employees/' + id, {
      ...
      var formattedDate = birthdate.getFullYear() + "-" + (month) + "-" + (day);
      this.setState({ id: data.id, fullName: data.fullName, birthdate: formattedDate, tin: data.tin, employeeTypeId: data.employeeTypeId, loading: false, loadingSave: false });
    }
